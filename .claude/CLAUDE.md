...
```bash
mgrep "ta question en langage naturel" --store "lights" -a -m <nombre>
```
...
|-----------|-------------|
| `--store "lights"` | **Obligatoire** - le store indexé du projet |
...
mgrep "comment fonctionne l'authentification GitHub côté frontend" --store "lights" -a -m 20
mgrep "comment le token est géré côté serveur" --store "lights" -a -m 20
mgrep "comment les sessions sont gérées" --store "lights" -a -m 20
```
